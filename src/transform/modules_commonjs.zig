...
fn exitNode(idx: NodeIndex, ctx: *TransformContext) visitor.VisitResult {
    if (ctx.scope) |_| {
        const rendered = lowering.renderCommonJSProgram(ctx, idx) orelse return .skip_children;
    ...
    const rendered = lowering.renderCommonJSProgram(&local_ctx, idx) orelse return .skip_children;
    ctx.putReplacementSource(idx, rendered) catch {};
    return .skip_children;
}
